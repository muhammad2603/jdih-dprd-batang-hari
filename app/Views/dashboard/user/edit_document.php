...
<?php
helper('string');
if (!is_null(esc($produk_hukum["berkas"]))) {
    $split_attachments = explode(",", esc($produk_hukum["berkas"]));
...
<section class="form-wrapper max-w-4xl mx-auto">
    <div class="form-header flex items-center gap-4 mb-6">
        ...
        <div class="form-title w-full">
            ...
            <p class="text-gray-500 text-sm mt-0.5">Mengedit: <?= esc($produk_hukum["judul"]) ?></p>
        </div>
    </div>
    <div class="form-body">
        <div class="bg-white rounded-t-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="tabs-form flex border-b border-gray-100">
                ...
                <button type="button" data-tab-id="abstract" class="abstract flex items-center gap-2 px-5 py-3.5 text-sm font-medium border-b-2 transition-all cursor-pointer border-transparent text-gray-500 hover:text-gray-700">
                ...
                <button type="button" data-tab-id="relatedDocuments" class="related-documents flex items-center gap-2 px-5 py-3.5 text-sm font-medium border-b-2 transition-all cursor-pointer border-transparent text-gray-500 hover:text-gray-700">
                    <svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-4">
                        <use href="/assets/icons.svg#icon-document">
                    </svg>
                    <span>Dokumen Terkait</span>
                </button>
                <button type="button" data-tab-id="attachmentSelect" class="attachments flex items-center gap-2 px-5 py-3.5 text-sm font-medium border-b-2 transition-all cursor-pointer border-transparent text-gray-500 hover:text-gray-700">
            ...
            <div class="form-body p-6">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="penandatanganan">
                                ...
                                <select name="penandatanganan" id="penandatanganan" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white border-gray-200">
                                    <?php foreach ($pejabat as $p): ?>
                                        <option value="<?= esc($p["id"]) ?>" <?= (esc($p["nama"]) === esc($produk_hukum["pejabat_penandatanganan"]) ? "selected" : "") ?>><?= esc($p["nama"]) ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                ...
                <div id="abstract" class="tab space-y-4 hidden">
                    <div id="inputFileAbstractWrapper" class="<?= $abstract_pdf ? 'hidden' : '' ?>">
                    ...
                    <div id="fileAbstractSelected" class="file-abstract-selected p-4 bg-gray-50 rounded-xl <?= !$abstract_pdf ? 'hidden' : '' ?>">
                            ...
                            <?php if (count($related_documents) > 0): ?>
                                <?php foreach ($related_documents as $rd): ?>
                                    <div data-related-id="<?= esc($rd["id"]) ?>" class="related-document-inputs w-full flex items-end gap-3">
                                        ...
                                        <button type="button" title="Hapus" class="btn-delete-related p-2 rounded-lg hover:bg-red-100 text-gray-400 hover:text-red-600 transition-colors mt-0.5 cursor-pointer">
                                            <svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-4">
                                                <use href="/assets/icons.svg#icon-trash-strip" />
                                            </svg>
                                        </button>
                                ...
                                <?php endforeach ?>
                            <?php endif ?>
                            <div class="related-document-inputs new w-full flex items-end gap-3">
                                <div class="flex flex-col gap-1 shrink-0 grow basis-2xs">
                ...
                <div id="attachmentSelect" class="tab space-y-4 hidden">
                    <div class="flex items-center justify-end gap-3  <?= is_null(esc($produk_hukum["berkas"])) ? 'hidden' : '' ?>">
                    ...
                    <div class="attachment-input-wrapper <?= !is_null(esc($produk_hukum["berkas"])) ? 'hidden' : '' ?>">
                    ...
                    <div class="histories-input">
                        <div class="py-3 px-4 bg-gray-50 rounded-xl">
                            <div class="w-full flex gap-3">
                                ...
                                <select id="changeType" class="px-2 py-2 text-sm border rounded-lg outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-white border-gray-200 disabled:bg-input-disabled">
                                    <?php foreach ($tipe_perubahan as $tipe): ?>
                                        <option value="<?= esc($tipe["id"]) ?>"><?= esc($tipe["tipe"]) ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                        </div>
