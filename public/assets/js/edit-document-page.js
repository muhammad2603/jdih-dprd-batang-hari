...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const produkHukumId = $('meta[name=PRODUK-HUKUM-ID]').getAttribute('value');
    ...
    btnSaveChanges.addEventListener("click", () => {
        ...
        const bidangHukumChanges = {
            ...
            deleted: currentDynamicValues["bidang_hukum"].filter(id => !newBidangHukumSet.has(id))
        };
        ...
        const subjekChanges = {
            ...
            deleted: currentDynamicValues["subjek"].filter(id => !newSubjekSet.has(id))
        };
        ...
        Object.entries(currentDynamicValues["dokumen_terkait"]).forEach(([idx, obj]) => {
            ...
            if (relatedDocument === null) {
                ...
                if (isRelatedDocumentChanged) {
                    delete values["nomor_tahun_dokumen_terkait"];
                    values["id"] = id;
                    values["nomor_dokumen_terkait"] = number;
                    values["tahun_dokumen_terkait"] = year;
        ...
        for (let idx = 0; idx < getInputsTitleAttachment.length; idx++) {
            ...
            if (!isTitleAttachmentChange) continue;
        ...
        // Blok ini akan selalu jalan terlepas file abstrak sudah ada atau tidak, jika file abstrak sudah ada, otomatis akan dihapus.
        if (fileAbstract !== null) {
            fileAbstractModify.add = fileAbstract;
        }
        ...
        // Block if ini mencegah saat tidak ada perubahan sama sekali.
        if (
        ...
        if (changesValue["nomor_tahun_dokumen"] !== undefined) {
            ...
            } else if (!validations(year).isInvalidValue(undefined) && validations(year).isValidValueLength(4, 4)) { // Menggunakan isInvalid diperlukan, karena, jika year memiliki nilai undefined, tidak akan bisa diambil lengthnya.
                isValid = false;
                $$(noTahunError).text("Format tahun dokumen adalah 4 digit. Contoh: 2020, 2021, dst.")
            }
            delete changesValue["nomor_tahun_dokumen"];
            changesValue["nomor"] = number;
            changesValue["tahun"] = year;
        }
        if (changesValue["nomor_tahun_tld"] !== undefined) {
            ...
            } else if (!validations(yearTld).isInvalidValue(undefined) && validations(yearTld).isValidValueLength(4, 4)) { // Menggunakan isInvalid diperlukan, karena, jika yearTld memiliki nilai undefined, tidak akan bisa diambil lengthnya.
                isValid = false;
                $$(noTahunTldError).text("Format tahun TLD dokumen adalah 4 digit. Contoh: 2020, 2021, dst.")
            }
            delete changesValue["nomor_tahun_tld"];
            changesValue["nomor_tld"] = numberTld;
            changesValue["tahun_tld"] = yearTld;
        }
        ...
        Object.entries(changesValue).forEach(([field, value]) => formData.append(field, value))
        bidangHukumChangesEntries.forEach(([operation, data]) => formData.append(`bidang_hukum[${operation}]`, JSON.stringify(data)))
        subjekChangesEntries.forEach(([operation, data]) => formData.append(`subjek[${operation}]`, JSON.stringify(data)))
        fileAbstractModifiedEntries.forEach(([operation, value]) => formData.append(`file_abstrak[${operation}]`, value))
        ...
        if (!isHistoryCommentDisabled) {
            formData.append('tipe_perubahan', $$(changeType).getInputValue())
            formData.append('riwayat_perubahan', historyCommentValue)
        }
        formData.append('_method', 'PATCH');

        fetch(`/api/update-dokumen/${produkHukumId}`, {
            method: "POST",
            headers: {
                "X-REQUESTED-WITH": 'XMLHttpRequest',
                "X-CSRF-TOKEN": $$(tokenCsrf).getInputValue(),
            },
            body: formData
        })
            .then(async resp => {
                const response = await resp.json();
                if (!resp.ok || response.status === false) throw response;
                return response;
            })
            .then(data => {
                const { status } = data;
                if (status) {
                    window.location.href = '/user/dashboard/kelola-dokumen';
                }
            })
            .catch(err => {
