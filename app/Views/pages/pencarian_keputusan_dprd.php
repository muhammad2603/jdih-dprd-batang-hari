...
<div class="jumbotron bg-primary text-white py-16">
    <div class="max-w-7xl mx-auto px-6">
        <div class="animate">
            <h1 class="text-2xl sm:text-3xl xl:text-4xl font-bold mb-4 md:leading-12 text-pretty">Keputusan DPRD</h1>
            <p class="text-sm sm:text-lg text-white/80 max-w-2xl">Kumpulan Keputusan DPRD Kabupaten Batang Hari terkait fungsi dan kewenangan dewan</p>
        </div>
    </div>
</div>
<div class="contents-container max-w-7xl mx-auto px-6 py-12">
    <div class="about-document bg-white border border-primary-border rounded-lg p-6 mb-8">
        <h2 class="font-semibold mb-3">Tentang Keputusan DPRD</h2>
        <p class="text-muted-foreground text-sm leading-relaxed">
            Keputusan DPRD adalah produk hukum yang ditetapkan oleh Dewan Perwakilan Rakyat Daerah dalam melaksanakan fungsi dan kewenangannya. Keputusan DPRD dapat berupa penetapan pimpinan alat kelengkapan dewan, pembentukan panitia khusus, persetujuan terhadap rancangan peraturan daerah, dan keputusan lain yang berkaitan dengan tugas dan fungsi DPRD sebagaimana diatur dalam peraturan perundang-undangan.
        </p>
    </div>
    <div class="filter-search-wrapper bg-white border border-primary-border rounded-lg p-6 mb-8">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div class="search relative md:col-span-3">
                ...
                <input id="searchInput" type="text" value="<?= uri_title_to_words($current_search) ?>" placeholder="Cari kata kunci keputusan dprd..." class="w-full pl-12 pr-4 py-4 bg-input-background rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
    ...
    <div class="documents">
        <p class="text-muted-foreground">Menampilkan <?= $data_display_count ?> dari <?= $total_dokumen ?> Keputusan DPRD yang ditemukan</p>
        <div class="mt-6 space-y-4">
            <?php foreach ($dokumen as $kep_dprd): ?>
                        ...
                        <div class="flex-1">
                            <div class="document-details flex items-center gap-3 flex-wrap mb-3">
                                <div class="category w-full sm:w-auto flex shrink-0 gap-3">
                                    ...
                                    <span class="text-sm font-medium text-primary"><?= esc($kep_dprd["kategori"]) ?></span>
                                </div>
                                <span class="text-sm text-muted-foreground">Nomor <?= esc($kep_dprd["nomor"]) ?> Tahun <?= esc($kep_dprd["tahun"]) ?></span>
                                <span class="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700"><?= esc($kep_dprd["status"]) ?></span>
                            </div>
                            <h3 class="font-semibold text-default-foreground mb-2 group-hover:text-primary transition-colors"><?= esc($kep_dprd["judul"]) ?></h3>
                            <div class="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <span>Ditetapkan: <time datetime="<?= esc($kep_dprd["tanggal_penetapan"]) ?>"><?= $timeServices->translateDateToLocalFormat(esc($kep_dprd["tanggal_penetapan"])) ?></time></span>
                            </div>
                        </div>
                        <div class="flex gap-2 shrink-0 self-end lg:self-start">
                            <a href="<?= "/produk-hukum/" . url_title(esc($kep_dprd["kategori"]), "-", true) . "/" . esc($kep_dprd["slug"]) ?>" class="flex items-center gap-2 py-2.5 px-4 xl:py-2 xl:px-2 text-primary hover:bg-primary/10 active:bg-primary/10 rounded-lg transition-colors focus:outline-none">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-4">
                                    <path d="M12 7v14" />
                                    <path d="M16 12h2" />
                                    <path d="M16 8h2" />
